...
4. **Create the employee's knowledge folder** in your project (e.g. `company/`) and put their working data there: contracts, sales exports, expense sheets, a `how-we-work.md`. Agents never get data "uploaded" — they read files you point them at.
5. **Place the finished file** in your project's `.claude/agents/` (this project only) or `~/.claude/agents/` (all projects). Restart the session. Done — Claude now delegates matching work to them automatically.
