...
async function runTests() {
  ...
  if (test('all hook commands use node with a graceful no-node guard', () => {
    const hooksPath = path.join(__dirname, '..', '..', 'hooks', 'hooks.json');
    const hooks = JSON.parse(fs.readFileSync(hooksPath, 'utf8'));
        ...
        for (const hook of entry.hooks) {
          if (hook.type === 'command') {
            assert.ok(
              ...
              `Hook command should invoke node: ${hook.command.substring(0, 50)}...`
            );
            assert.ok(
              hook.command.includes('command -v node >/dev/null 2>&1 || exit 0;'),
              `Hook command should no-op when node is missing: ${hook.command.substring(0, 50)}...`
            );
            assert.ok(
              hook.command.startsWith('PATH="$HOME/.local/node/bin:'),
              `Hook command should extend PATH to find user-space node: ${hook.command.substring(0, 50)}...`
            );
          }
        }
      }
